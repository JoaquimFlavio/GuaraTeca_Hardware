...
void SensorCor::iniciaSensorCor(uint8_t out, uint8_t S0, uint8_t S1, uint8_t S2, uint8_t S3){
    this->_S0 = S0;
    this->_S1 = S1;
    this->_S2 = S2;
    this->_S3 = S3;
...
void SensorCor::filtroVermelho(){
    digitalWrite(_S2, LOW);
    digitalWrite(_S3, LOW);
}
void SensorCor::filtroVerde(){
    digitalWrite(_S2, HIGH);
    digitalWrite(_S3, HIGH);
}
void SensorCor::filtroAzul(){
    digitalWrite(_S2, LOW);
    digitalWrite(_S3, HIGH);
}
void SensorCor::filtroNulo(){
    digitalWrite(_S2, HIGH);
    digitalWrite(_S3, LOW);
}
void SensorCor::frequencia(uint8_t OPT){
    switch(OPT){
        case 1://2%
...
float SensorCor::leitura(){
    return pulseIn(_out, digitalRead(_out) == HIGH ? LOW : HIGH);
}
